...
{//对Mem_Mgr的内容验算
	...
	{
		p = (int*)&poMem_Mgr->m_pBuffer[poMem_Mgr->m_pHash_Item[i].m_iPos * (unsigned long long)poMem_Mgr->m_iBytes_Per_Bottom_Sub_Block];
		for (j = 0; j < poMem_Mgr->m_pHash_Item[i].m_iSub_Block_Count * poMem_Mgr->m_iBytes_Per_Bottom_Sub_Block / 4; j++)
		{
			if (p[j] != j)
			{
				printf("Piece开始块:%d 第 %d 项的第 %d 个元素出错，本应：%d 错误:%d , 位置: Buffer[%lld] ", (int)poMem_Mgr->m_pHash_Item[i].m_iPos, i, j, j, p[j], (unsigned long long)((unsigned char*)&p[j] - poMem_Mgr->m_pBuffer));
				exit(0);
			}
		}
		printf("第 %d 项通过\n", i-1);
...
void Random_Test_2()
{//真正的随机测试，这个过了应该差不多了
#define MAX_PIECE_COUNT 65521
#define MAX_SUB_BLOCK 1024
	...
	for (i = MAX_PIECE_COUNT - 1; i >= 0; i -= 2)
	{
		if (Piece_Size[i])
		{
			...
			Allocate[i] = pMalloc_1(&oMem_Mgr, Piece_Size[i]);
			//if (Allocate[i])
...
#define MAX_PIECE_COUNT 1023
	...
	Mem_Mgr oMem_Mgr;
	Init_Mem_Mgr(&oMem_Mgr, 14000000023, 1024, MAX_PIECE_COUNT);
	Piece_Size[0] = (unsigned int)(oMem_Mgr.m_Layer[2].m_iBytes_Per_Block * 2 + 1024*33);
	Allocate[0] = pMalloc(&oMem_Mgr, Piece_Size[0]);
	Piece_Size[1] = (unsigned int)(oMem_Mgr.m_Layer[2].m_iBytes_Per_Block * 20);
	Allocate[1] = pMalloc(&oMem_Mgr, Piece_Size[1]);
	Piece_Size[2] = 1024;
	Allocate[2] = pMalloc(&oMem_Mgr, Piece_Size[2]);
	...
	bExpand(&oMem_Mgr, Allocate[0], Piece_Size[0]+1024*32);
	Piece_Size[0] = (unsigned int)(Piece_Size[0] + 1024*32);
	Set_Value((unsigned int*)Allocate[0], Piece_Size[0] / 4);
	Test_Mem_Mgr(&oMem_Mgr);
